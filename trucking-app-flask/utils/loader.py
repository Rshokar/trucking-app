...
def loadDB(num_users):
    ...
    for index in range(num_users): 
        ...
        customers = CustomerFactory.create_batch(5, company_id=company.company_id)
      
        operators = OperatorFactory.create_batch(5, company_id=company.company_id)

        dispatches =  []
        rfos = []
        billing_tickets = []
        for customer in customers:
            d = DispatchFactory.create_batch(5, company_id=company.company_id, customer_id=customer.customer_id)
            
            for dispatch in d:  
                numRfos = random.randint(0, len(operators))
                for i in range(numRfos):
                    rfo = RFOFactory.create(dispatch_id=dispatch.dispatch_id, operator_id=operators[i].operator_id)
...
class UserFactory(SQLAlchemyModelFactory):
    ...
    def generate_password():
        ...
        while not (any(c.isdigit() for c in password) and any(c.isupper() for c in password) and any(c.islower() for c in password)):
            password = fake.pa('passwor', length=8, special_chars=False, digits=True, upper_case=True, lower_case=True)
        return password
...
class CompanyFactory(SQLAlchemyModelFactory):
    ...
    name = Faker("company")
    company_id = Sequence(lambda n: n + 1)
...
class OperatorFactory(SQLAlchemyModelFactory):
    ...
    operator_id = Sequence(lambda n: n + 1)
    company_id = SubFactory(CompanyFactory)
...
class BillingTicketFactory(SQLAlchemyModelFactory):
    ...
    def generate_random_image_id():
        sections = []
        for i in range(4):
            section = ''.join(random.choices('0123456789abcdefghijklmnopqrstuvwxyz', k=6))
            sections.append(section)
        return '-'.join(sections)
    
    rfo_id = SubFactory(RFOFactory)
    ticket_number = Sequence(lambda n : n + 1)
    image_id = LazyAttribute(lambda obj: BillingTicketFactory.generate_random_image_id())
